...
function fetchBorrowDetails($conn)
{
    $query = "SELECT bl.borrow_id, bl.borrow_date, bl.return_date, b.book_name, m.username 
              FROM borrow_list bl 
...
$borrowDetails = fetchBorrowDetails($conn);
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="bg-gray-50">
    ...
    <aside class="fixed left-0 top-0 w-64 h-full bg-white border-r border-gray-200 z-30">
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="manage_borrow.php" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-indigo-50 text-indigo-600">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"></path>
                        <path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"></path>
                    </svg>
        ...
        <header class="bg-white border-b border-gray-200 px-8 py-4">
            ...
            <section class="bg-white p-6 rounded-md shadow-md">
                ...
                <table class="min-w-full bg-white border border-gray-200 rounded-md">
                    ...
                    <tbody>
                    <?php while ($borrow = $borrowDetails->fetch_assoc()): ?>
                                <tr>
                                    <td class="py-2 px-4 border-b"><?= $borrow['borrow_id'] ?></td>
                                    <td class="py-2 px-4 border-b"><?= $borrow['book_name'] ?></td>
                                    <td class="py-2 px-4 border-b"><?= $borrow['username'] ?></td>
                                    <td class="py-2 px-4 border-b"><?= $borrow['borrow_date'] ?></td>
                                    <td class="py-2 px-4 border-b"><?= $borrow['return_date'] ?></td>
                                    <td class="py-2 px-4 border-b">
                                        <a href="javascript:void(0);" onclick="confirmDelete('delete_borrow.php?id=<?= $borrow['borrow_id'] ?>')" class="text-red-500"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                    </tbody>
