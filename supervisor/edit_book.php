...
if (isset($_GET['id'])) {
    ...
    if (!$book) {
        echo '<script>alert("Book details not found."); window.location.href = "manage_books.php";</script>';
        exit();
    }
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['book_id'], $_POST['book_name'], $_POST['author_name'], $_POST['published_date'],$_POST['branch'], $_POST['category'], $_POST['stock'], $_POST['rack'], $_POST['des'])) {
        ...
        if ($book_cover) {
            ...
            // Fetch the previous book cover from the database
            $query = "SELECT book_cover FROM books WHERE book_id = ?";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("s", $book_id);
            $stmt->execute();
        ...
        $query = "UPDATE books SET book_name = ?, author_name = ?, published_date = ?,branch=?, category = ?, stock = ?, rack = ?, book_cover = ?, description = ? WHERE book_id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssssssss", $book_name, $author_name, $published_date, $branch, $category, $stock, $rack, $book_cover, $des, $book_id);
        $stmt->execute();
            ...
            <div class="p-8">
                <section class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    <form action="edit_book.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label for="branch" class="block text-sm font-medium text-gray-700">Category</label>
                            <input type="text" id="branch" name="branch" value="<?php echo $book['branch']; ?>" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                        <div class="mb-4">
