...
<head>
    ...
    <title>Manage Fines - Library App</title>
    ...
    <div class="min-h-screen">
        <aside id="sidebar" class="fixed left-0 top-0 w-64 h-full bg-white border-r border-gray-200 z-30 hidden">
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="manage_fines.php" class="flex items-center gap-3 p-2 text-gray-700 hover:bg-gray-100 rounded-lg active">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <main id="main-content" class="p-6">
            <header class="bg-white border-b border-gray-200 px-8 py-4 flex justify-between items-center">
                ...
                <div class="relative w-96">
                    <form method="GET" action="manage_fines.php" class="relative">
                        <input
                            type="text"
                            name="search"
            ...
            <section class="bg-white p-6 rounded-md shadow-md">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-xl font-semibold">Manage Fines</h2>
                </div>
                <table class="min-w-full bg-white border border-gray-200 rounded-md">
                    <thead>
                        <tr>
                            <th class="py-2 px-4 border-b">Fine ID</th>
                    ...
                    <tbody>
                        <?php if (is_object($fines) && $fines->num_rows > 0): ?>
                            <?php while ($fine = $fines->fetch_assoc()): ?>
                                <tr>
                                    <td class="py-2 px-4 border-b"><?= $fine['fine_id'] ?></td>
                                    <td class="py-2 px-4 border-b"><?= $fine['book_name'] ?></td>
                                    <td class="py-2 px-4 border-b"><?= $fine['username'] ?></td>
                                    <td class="py-2 px-4 border-b"><?= $fine['fine_amount'] ?></td>
                                    <td class="py-2 px-4 border-b"><?= $fine['due_date'] ?></td>
                                    <td class="py-2 px-4 border-b">
                                        <a href="edit_fine.php?id=<?= $fine['fine_id'] ?>" class="text-blue-500"><i class="fa fa-pen"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;
                                        <a href="delete_fine.php?id=<?= $fine['fine_id'] ?>" class="text-red-500"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan='6' class="text-center py-4">No fines found.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </section>
