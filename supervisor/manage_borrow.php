...
$fines = fetchFines($conn); 
$supervisorProfile = fetchUserProfile($conn, $_SESSION['email']); // Fetch admin profile data
...
<head>
    ...
    <title>Manage Borrow - Library App</title>
    ...
    <div class="min-h-screen">
        <aside id="sidebar" class="fixed left-0 top-0 w-64 h-full bg-white border-r border-gray-200 z-30 hidden">
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="manage_borrow.php" class="flex items-center gap-3 p-2 text-gray-700 hover:bg-gray-100 rounded-lg active">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <main id="main-content" class="p-6">
            <header class="bg-white border-b border-gray-200 px-8 py-4 flex justify-between items-center">
                ...
                <div class="relative w-96">
                    <form method="GET" action="manage_borrow.php" class="relative">
                        <input
                            type="text"
                            name="search"
            ...
            <section class="bg-white p-6 rounded-md shadow-md">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-xl font-semibold">Manage Borrow</h2>
                </div>
                <table class="min-w-full bg-white border border-gray-200 rounded-md">
                    <thead>
                        <tr>
                            <th class="py-2 px-4 border-b">Borrow ID</th>
                            <th class="py-2 px-4 border-b">Book Id</th>
                            <th class="py-2 px-4 border-b">Borrower Pin Number </th>
                    ...
                    <tbody>
                        ...
                        $query = "SELECT borrow_id, book_id, member_id, borrow_date, return_date, status FROM borrow_list"; // Adjust the table name as necessary
                        $borrowDetails = $conn->query($query); 
                        ...
                        while ($borrow = $borrowDetails->fetch_assoc()): ?>
                            <tr>
                                <td class="py-2 px-4 border-b"><?= $borrow['borrow_id'] ?></td>
                                <td class="py-2 px-4 border-b"><?= $borrow['book_id'] ?></td>
                                <td class="py-2 px-4 border-b"><?= $borrow['member_id'] ?></td>
                                <td class="py-2 px-4 border-b"><?= $borrow['borrow_date'] ?></td>
                                <td class="py-2 px-4 border-b"><?= $borrow['return_date'] ?></td>
                                <td class="py-2 px-4 border-b"><?= $borrow['status'] ?></td>
                                <td class="py-2 px-4 border-b">
                                    <a href="edit_borrow.php?id=<?= $borrow['borrow_id'] ?>" class="text-blue-500"><i class="fa fa-pen"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;
                                    <a href="delete_borrow.php?id=<?= $borrow['borrow_id'] ?>" class="text-red-500"><i class="fa fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
        ...
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div class="space-y-4">
                <div class="flex justify-between py-2 border-b border-gray-100">
                    ...
                    <span class="font-medium text-gray-900"><?= htmlspecialchars($supervisorProfile     ['username']) ?></span>
                </div>
                <div class="flex justify-between py-2 border-b border-gray-100">
                    <span class="text-gray-600">Email</span>
                    <span class="font-medium text-gray-900"><?= htmlspecialchars($supervisorProfile     ['email']) ?></span>
                </div>
                <div class="flex justify-between py-2 border-b border-gray-100">
                    <span class="text-gray-600">Phone Number</span>
                    <span class="font-medium text-gray-900"><?= htmlspecialchars($supervisorProfile     ['phone_number']) ?></span>
                </div>
                <div class="flex justify-between py-2 border-b border-gray-100">
                    <span class="text-gray-600">Branch</span>
                    <span class="font-medium text-gray-900"><?= htmlspecialchars($supervisorProfile     ['branch']) ?></span>
                </div>
                <div class="flex justify-between py-2 border-b border-gray-100">
                    <span class="text-gray-600">Section</span>
                    <span class="font-medium text-gray-900"><?= htmlspecialchars($supervisorProfile     ['section']) ?></span>
                </div>
            </div>
        </div>
    </div>
