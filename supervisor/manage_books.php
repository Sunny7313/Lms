...
$adminProfile = fetchUserProfile($conn, $email);
...
if (isset($_GET['search'])) {
    ...
    if ($searchQuery !== '') {
        ...
        if ($searchResults === false || (is_array($searchResults) && count($searchResults) === 0)) {
            $noResultsMessage = "No results found for '$searchQuery'.";
        }
    }
} 

$books = searchBooks($conn);
if ($books === false) {
    error_log("Error fetching books: " . htmlspecialchars($conn->error));
    echo "<div class='error'>Error fetching books.</div>";
}
    ...
    <div class="min-h-screen">
        <aside id="sidebar" class="fixed left-0 top-0 w-64 h-full bg-white border-r border-gray-200 z-30 hidden">
            ...
            <nav class="p-4 space-y-2">
                <a href="index.php" class="flex items-center gap-2 p-2 text-gray-700 hover:bg-gray-100 rounded-lg">
                ...
                <a href="manage_books.php" class="flex items-center gap-2 p-2 text-gray-700 hover:bg-gray-100 rounded-lg active">
                ...
                <a href="manage_book_requests.php" class="flex items-center gap-2 p-2 text-gray-700 hover:bg-gray-100 rounded-lg">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <a href="manage_borrow.php" class="flex items-center gap-2 p-2 text-gray-700 hover:bg-gray-100 rounded-lg">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <a href="manage_fines.php" class="flex items-center gap-2 p-2 text-gray-700 hover:bg-gray-100 rounded-lg">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <tbody>
                        <?php if ($searchQuery !== ''): ?>
                            <?php if (!empty($noResultsMessage)): ?>
                                <tr>
                                    <td colspan="8" style="text-align: center;"><?= $noResultsMessage ?></td>
                                </tr>
                            <?php else: ?>
                                <?php while ($book = $searchResults->fetch_assoc()): ?>
                                    <tr>
                                        <td class="py-2 px-4 border-b"><?= htmlspecialchars($book['book_id']) ?></td>
                                        <td class="py-2 px-4 border-b"><?= htmlspecialchars($book['book_name']) ?></td>
                                        <td class="py-2 px-4 border-b"><?= htmlspecialchars($book['author_name']) ?></td>
                                        <td class="py-2 px-4 border-b"><?= htmlspecialchars($book['published_date']) ?></td>
                                        <td class="py-2 px-4 border-b"><?= htmlspecialchars($book['category']) ?></td>
                                        <td class="py-2 px-4 border-b"><?= htmlspecialchars($book['rack']) ?></td>
                                        <td class="py-2 px-4 border-b"><?= htmlspecialchars($book['stock']) ?></td>
                                        <td class="py-2 px-4 border-b">
                                            <a href="edit_book.php?id=<?= $book['book_id'] ?>" class="text-blue-500"><i class="fa fa-pen"></i></a>&nbsp;&nbsp;
                                            <a href="delete_book.php?id=<?= $book['book_id'] ?>" class="text-red-500"><i class="fa fa-trash"></i></a>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php endif; ?>
                        <?php elseif ($books && $books->num_rows > 0): ?>
