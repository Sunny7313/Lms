...
try {
    $query = "SELECT bl.borrow_id, bl.borrow_date, bl.return_date, bl.stock, b.book_id, b.book_name, m.pin_number, m.username 
              FROM borrow_list bl 
...
<head>
    ...
    <title>Borrowing Data - Library App</title>
    ...
    <div class="min-h-screen">
        ...
        <main id="main-content" class="main p-6">
            ...
            <section class="bg-white p-6 rounded-md shadow-md">
                <h2 class="text-xl font-semibold mb-4">Borrowing Data</h2>
                    ...
                    <table class="min-w-full bg-white border border-gray-200 rounded-md">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-2 border-b">Borrow Date</th>
                                <th class="px-4 py-2 border-b">Return Date</th>
                                <th class="px-4 py-2 border-b">Borrower Name</th>
                                <th class="px-4 py-2 border-b">Show Status/View Details</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            <?php $i = 1;
                            while ($borrow = $borrowingData->fetch_assoc()): ?>
                                <tr>
                                    <td class="px-4 py-2 border-b"><?= $i ?></td>
                                    <td class="px-4 py-2 border-b"><?= $borrow['book_name'] ?></td>
                                    <td class="px-4 py-2 border-b"><?= $borrow['borrow_date'] ?></td>
                                    <td class="px-4 py-2 border-b"><?= $borrow['return_date'] ?></td>
                                    <td class="px-4 py-2 border-b"><?= $borrow['username'] ?></td>
                                    <td class="px-4 py-2 border-b">
                                        <a href="show_status.php?borrow_id=<?= $borrow['borrow_id'] ?>"
                                            class="btn btn-warning btn-sm"><i class="fa fa-info-circle"></i></a>
                                    </td>
                                </tr>
                            <?php $i++;
                            endwhile; ?>
