...
function fetchBookRequests($conn, $userId)
{
    $query = "SELECT br.request_id, br.book_id, br.status, b.book_name FROM book_requests br INNER JOIN books b ON br.book_id = b.book_id WHERE br.user_id = ?";
    $stmt = $conn->prepare($query);
...
$userId = $userProfile['pin_number'];
$bookRequests = fetchBookRequests($conn, $userId);
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="min-h-screen">
        <aside id="sidebar" class="sidebar">
            ...
            <nav class="nav flex flex-col p-4">
                ...
                <a href="book_requests_status.php" class="nav-link flex items-center p-2 text-gray-700 hover:bg-gray-100 rounded-md active">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" class="mr-2">
        ...
        <main id="main-content" class="main p-6">
            ...
            <section class="bg-white p-6 rounded-md shadow-md">
                <h2 class="text-xl font-semibold mb-4">Your Book Requests</h2>
                <table class="min-w-full bg-white border border-gray-200 rounded-md">
                    <tr>
                        <th class="px-4 py-2 border-b">Request ID</th>
                        <th class="px-4 py-2 border-b">Book Name</th>
                        <th class="px-4 py-2 border-b">Status</th>
                        <th class="px-4 py-2 border-b">Action</th>
                    </tr>
                    <?php while ($request = $bookRequests->fetch_assoc()): ?>
                        <tr>
                            <td class="px-4 py-2 border-b"><?= htmlspecialchars($request['request_id']) ?></td>
                            <td class="px-4 py-2 border-b"><?= htmlspecialchars($request['book_name']) ?></td>
                            <td class="px-4 py-2 border-b"><?= htmlspecialchars($request['status']) ?></td>
                            <td class="px-4 py-2 border-b">
                                <?php if ($request['status'] === 'pending'): ?>
                                    <a href="cancel_request.php?request_id=<?= $request['request_id'] ?>"><i class="fa fa-trash" style="color:red;"> </i></a>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endwhile; ?>
